...
class ContactEditPage extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  async fetchContact() {
    ...
    if (response.isSuccess()) {
      ...
      this.setStatePartial({ first_name, last_name, email, phone });
    } else {
  ...
  async handleSubmit(event) {
    ...
    const { token, id, first_name, last_name, email, phone } = this.state;
    const response = await contactApi.updateContact(token, {
      id,
      first_name,
      last_name,
      email,
    ...
    if (response.isSuccess()) {
      await alertService.success("Contact updated successfully");
        ...
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-800 flex items-center">
            <div className="w-8 h-8 bg-gradient rounded-lg flex items-center justify-center mr-3">
              <i className="fas fa-user-edit text-white text-sm"></i>
            </div>
            Edit Contact
          </h1>
        </div>
...
const ContactEditPageWithRouter = withRouter(ContactEditPage);

export default ContactEditPageWithRouter;
